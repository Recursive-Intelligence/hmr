...
flags.DEFINE_string(
    "smpl_model_path", SMPL_MODEL_PATH, "path to the neurtral smpl model"
)
flags.DEFINE_string(
    "smpl_face_path", SMPL_FACE_PATH, "path to smpl mesh faces (for easy rendering)"
)
flags.DEFINE_string("load_path", None, "path to trained model")
flags.DEFINE_string(
    "pretrained_model_path", None, "if not None, fine-tunes from this ckpt"
)
flags.DEFINE_integer(
    "batch_size", 8, "Input image size to the network after preprocessing"
)
...
flags.DEFINE_string(
    "model_type", "resnet_fc3_dropout", "Specifies which network to use"
)
flags.DEFINE_string(
    "joint_type",
    "cocoplus",
...
flags.DEFINE_string(
    "model_dir", None, "Where model will be saved -- filled automatically"
)
...
flags.DEFINE_list(
    "datasets", ["lsp", "lsp_ext", "mpii", "coco"], "datasets to use for training"
)
flags.DEFINE_list(
    "mocap_datasets",
    ["CMU", "H3.6", "jointLim"],
...
flags.DEFINE_boolean(
    "encoder_only", False, "if set, no adversarial prior is trained = monsters"
)
    ...
    if config.load_path:
        ...
        for k in diff_keys:
            ...
            if prev_config[k] is None and dict_here[k] is not None:
                print("%s is different!! before: None after: %g" % (k, dict_here[k]))
            elif prev_config[k] is not None and dict_here[k] is None:
                print("%s is different!! before: %g after: None" % (k, prev_config[k]))
            else:
        ...
        default_dataset = ["lsp", "lsp_ext", "mpii", "h36m", "coco", "mpi_inf_3dhp"]
